...
- **Deep class coverage** — broken auth (default creds + user enum), GraphQL (introspection + clairvoyance schema recovery + alias/array batching + resolver injection → RCE/SQLi + **GET / form-urlencoded CSRF** surface), stored XSS, CSRF, SSTI→RCE (8 template engines), XXE (in-band file read), HTTP request smuggling (CL.TE/TE.CL + **TE-header obfuscation + cleartext-HTTP/2 `h2c` upgrade**), WebSocket CSWSH.
...
- **API authorization + SSRF** — BOLA/IDOR (confirms when other users' PII returns per id), JSON-body SSRF (learns body from validation errors, confirms in-band or via OOB) against AWS / GCP / Azure IMDS / Alibaba / DigitalOcean metadata (with decimal-IP filter-bypass). On a reachable AWS metadata service it **walks the full chain** — lists the instance IAM role, fetches its temporary credentials, and confirms a live `AccessKeyId` came back (masked in the report; never used) → SSRF → cloud-account takeover.
...
- **Business-logic flaws (stateful)** — a small flow engine confirms price/quantity tampering via a differential oracle (garbage *rejected* but a *negative* value *accepted* ⇒ type-only validation → negative totals / minted balance), monetary mass-assignment (client sets `price`/`total`/`balance` and it's stored), and reads the object back to prove the bad value persisted.
- **Password-reset / OTP / 2FA logic** — OTP or reset token leaked in the API response, missing rate-limit / lockout on OTP-code verification (brute-forceable → ATO, probed with throwaway identifiers so no real account is locked), and reset-link host-header poisoning (token delivered to an attacker host).
...
- **Headless browser** — Playwright renders SPAs, captures XHR/fetch endpoints, and confirms DOM-XSS execution (payload must actually fire — zero FP). Audits `window.postMessage` listeners for a missing origin check feeding a dangerous sink and **actively drives the handler** to prove execution (postMessage XSS confirmed, not just static). Confirms **client-side prototype pollution** by navigating `?__proto__[x]=y` and reading `Object.prototype` back live, and escalates it to a **PP → DOM-XSS gadget chain** when a polluted property reaches a library sink and executes the sentinel. Auto-skips if Playwright isn't installed.
...
- **Live secret validation** *(on by default; `--no-validate-secrets` to disable)* — replays a mined credential read-only against the API that issued it (GitHub, GitLab, Slack, Stripe, SendGrid, Mailgun, Square, Heroku, **npm, DigitalOcean, OpenAI, Telegram, Figma, Postman, Cloudflare, Google Maps**) and reports which secrets are **actually live**, the principal they authenticate as, **and what the key can do** (scopes / blast radius) vs. revoked — turning a *"possible secret"* into a confirmed critical. Contacts only allowlisted issuer hosts, never the target, with a fresh connection so your session is never leaked; raw secrets stay out of the report.
- **Technology exploit packs + KEV ranking** — once the stack is fingerprinted, runs targeted confirmed checks (WordPress REST user-enum + XML-RPC amplification, Jenkins unauthenticated Groovy `/script` console = RCE, Jira/Confluence/GitLab version disclosure) and cross-references the detected version against known-vulnerable builds. Findings referencing a **CISA-KEV** CVE (exploited in the wild) are floated to the top of the hunt list, so you test what attackers actually use first.
