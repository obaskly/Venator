...
def write_summary(report: dict, path: str) -> str:
    ...
    A("")
    A("> Authorized testing only. This run **autonomously exploits and confirms** "
      "vulnerabilities, attaching a working PoC to each `confirmed`/EXPLOITED finding "
      "(it captures a minimal proof rather than dumping data, so the PoC is "
      "report-ready). Remaining items are leads the tool could not auto-confirm — they "
      "say why and what is left to check.")
    A("")
...
<header>
  ...
  <div class="sub">autonomous exploitation · authorized testing only</div>
</header>
{warn_html}
...
<footer>Generated by reconscan · confirmed findings carry a PoC; leads note what is left to check</footer>
</div>
<script>
